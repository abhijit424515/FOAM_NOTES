...
mkdir -p alpha/a/1                  # creates nested folders in one go, starting from wherever a folder in the nest is not available
...
diff file1 file2                    # display differentiating lines when comparing file1 and file2

df                                  # displays partitions, their sizes, and their mount points

du                                  # list out space usage of each CONFIG FILE ???

scp <source> <destination>          # copy files from source system to destination system
